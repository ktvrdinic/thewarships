	...
	void Update()
	{
		...
		if (mFireTime != 0f && mFireTime <= time)
		{
			...
			if (cannonballPrefab != null)
			{
                ...
                GameObject go = PhotonNetwork.Instantiate(cannonballPrefab.name, mTrans.position, mTrans.rotation, 0) as GameObject;    
                //Instantiate(cannonballPrefab, mTrans.position, mTrans.rotation) as GameObject;
